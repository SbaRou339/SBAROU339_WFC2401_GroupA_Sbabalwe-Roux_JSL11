...
function displayBoards(boards) {
  ...
  boards.forEach(board => {
    ...
    boardElement.addEventListener("click", () => { // Corrected click event listener syntax
...
function filterAndDisplayTasksByBoard(boardName) {
  ...
  elements.columnDivs.forEach(column => {
    ...
    filteredTasks.filter(task => task.status === status).forEach(task => { 
      ...
      taskElement.addEventListener('click', () => { // Corrected click event listener syntax
...
function addTaskToUI(task) {
  const column = document.querySelector(`.column-div[data-status="${task.status}"]`); // Corrected string interpolation
  if (!column) {
...
function setupEventListeners() {
  ...
  cancelEditBtn.addEventListener('click', () => toggleModal(false, elements.editTaskModal)); // Corrected event listener syntax
...
function toggleModal(show, modal = elements.modalWindow) {
  modal.style.display = show ? 'block' : 'none'; // Fixed syntax and logical issue
}
